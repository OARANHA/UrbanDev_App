...
// material-ui
import { Stack, useTheme, Typography, Box, Alert, Button, Divider, Icon, Card, CardContent, IconButton } from '@mui/material'
...
const SignInSupabasePage = () => {
    ...
    const supabaseLoginApi = useApi(supabaseApi.supabaseSignIn)
    ...
    const doLogin = async (event) => {
        ...
        try {
            ...
            await supabaseLoginApi.request(body)
        } catch (error) {
            // Fallback to original auth if Supabase fails
            const body = {
                email: usernameVal,
                password: passwordVal
            }
            await loginApi.request(body)
        }
    }
    ...
    useEffect(() => {
        if (supabaseLoginApi.error) {
            setLoading(false)
            setAuthError(supabaseLoginApi.error.response?.data?.error || 'Erro ao fazer login')
        }
    }, [supabaseLoginApi.error])
    ...
    useEffect(() => {
        if (supabaseLoginApi.data) {
            setLoading(false)
            // Transform Supabase user to match expected format
            const userData = {
                ...supabaseLoginApi.data.user,
                name: supabaseLoginApi.data.user.name || supabaseLoginApi.data.user.email,
                apiKey: 'supabase-user', // Placeholder or generate actual API key
                isApiKeySet: true
            }
                ...
                <Card sx={{ maxWidth: 500, width: '100%', borderRadius: 3, boxShadow: '0 20px 40px rgba(0,0,0,0.1)' }}>
                    <CardContent sx={{ p: 4 }}>
                        <Stack sx={{ gap: 3 }}>
                            ...
                            {/* Divisor OAuth Supabase */}
                            <Stack sx={{ alignItems: 'center', gap: 2 }}>
                                <Divider sx={{ width: '100%', my: 2 }}>
                                    <Typography variant='body2' sx={{ color: theme.palette.text.secondary }}>
                                        ou continue com Supabase
                                    </Typography>
                                </Divider>
...
export default SignInSupabasePage